function data_extraction_SingleDualControl
...
if stand_alone == 1
    analysed_output_file = sprintf('%d_subs_extracted_SingleDualControl_data.xls',length(subject_numbers));
end;
...
% RUN CODE ----------------------------------------------------------------
    ...
    for session_count = 1:length(sessions_to_analyse)
        ...
        for subject_count = 1:length(subject_numbers)
            ...
            filename=strcat('Dual_Task_Training_SYMBOLS_PREPOST_SubjectNumber_',num2str(P),'_SessionNumber_',num2str(Sess),'.txt'); 
